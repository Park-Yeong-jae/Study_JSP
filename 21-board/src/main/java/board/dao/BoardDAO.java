...
//				+ "where rn>=? and rn<=?";
	...
	public int boardModify(BoardBean bean) {
		...
		String sql = "update board2 set board_name=?, "
					+ "board_subject=?, board_content=?, board_date=sysdate "
					+ "where board_num=?";
	...
	//답글저장 
	//기존글의 re_seq값 정리 : 같은 그룹에서 원글보다 큰 re_seq값을 1씩 증가
	//1-1. update board2 set board_re_seq = board_re_seq +1 where board_re_ref = 72 and board_re_seq>0;
	
	//답글저장(re_ref = 원글의 re_ref , re_lev = 원글의 re_lev+1, re_seq = 원글의 re_seq+1)
	//1-2. insert into board values (seq_num.nextval,'홍길동','1234','내일은','웃으리','aa.jpg',72,1,1,0,sysdate);	
	public int insertReplyArticle(BoardBean boardBean) {
		...
		try {
			conn = ds.getConnection();
			//1)기존글의 re_seq값 정리 : 같은 그룹에서 원글보다 큰 re_seq값을 1씩 증가
